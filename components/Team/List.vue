  ...
  <UCard>
    <div class="flex flex-col gap-5">
      <div class="flex gap-5 items-center ">
        <UFormGroup class="grow"> <UInput v-model="name_Q" placeholder="الاسم" /></UFormGroup>
        <UFormGroup label="ذات">
          <UToggle v-model="zat_Q" />
        </UFormGroup>
      </div>
      <!-- {{ Filterzat }} -->
      <UTable :rows="Filterzat" :columns="columns">
        <template #name-data="{ row }">
          <div class="flex items-center gap-5">
            <UAvatar size="xl" :src="row.logoUrl" />
            <h2 calss="texxt-xl ">{{ row.name }}</h2>
          </div>
        </template>
    ...
    <template #footer>
      <UButton to="/team/add">add </UButton>
    </template>
  </UCard>
</template>
...
const Filterzat = computed(() => {
  ...
  return  Filtername.value?.filter((t) => {
